...
if __name__=="__main__":
   ...
   # Parto da position = [0, 0, 0, 0, 0, 0]
   # Poi entro in un for loop, in cui x prende i seguenti valori : [-1,-2, ... ,-49,-50, 1,2, ... ,49,50]
   # A questo punto modifico la position andando a sommare o sottrarre un offset ad essa.
   # Esempio:
   # 		Initial position=[0,0,0,0,0,0]
   # 		Primo valore di x=-1
   # 		x è negativo --> l'offset viene moltiplicato per -1 --> l'offset è -0.1
   # 		Position=[0,0,0,0,0,0] + offset --> position = [-0.1,-0.1,-0.1,0.0,0.0,0.0]
   # 				(solo i primi 3 joints vengono cambiati)
	

	t_start = time.time()
	while(time.time() - t_start < duration):
		try:
			for x in range(-1, -50, -1) + range(1, 50, 1): # [range(-1, -50, -1)] + [range(1, 50, 1)] == [-1,-2, ... ,-49,-50, 1,2, ... ,49,50]
